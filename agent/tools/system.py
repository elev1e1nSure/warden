...
class SystemInfoTool(Tool):
    name = "system_info"
    description = (
        "Report machine info: OS, hostname, CPU count, total RAM, disk usage, uptime, Python version. Read-only."
    )
    params = {}
    ...
    async def execute(self, args: dict[str, Any]) -> str:
        ...
        for root in roots:
            try:
                ...
                lines.append(f"disk {root}: {_fmt_bytes(usage.used)} / {_fmt_bytes(usage.total)} used")
            except OSError:
                pass
        return "\n".join(lines)
