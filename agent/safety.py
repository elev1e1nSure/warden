...
def _is_path_within_workspace(path: str | Path, workspace: Path | None = None) -> bool:
    """True if *path* is contained inside *workspace* (case-insensitive on Windows)."""
    try:
...
_CURL_ALIASES = {"curl", "wget", "iwr", "invoke-webrequest", "irm", "invoke-restmethod"}

# Destructive flags
_RECURSE_FLAGS = {"-recurse", "-r", "/s", "-fo"}
_FORCE_FLAGS = {"-force", "-f", "/f", "-fo"}
...
_BLOCKED_CMDLETS = {
    ...
    "out-file",  # overwrite (handled via context)
...
_SAFE_CMDLETS = {
    ...
    "out-string", "out-null",
    "findstr", "grep", "rg", "fd",
    "python", "py",  # python is confirm by default, but safe for reads
}

# Patterns for encoded/remote execution
_ENCODED_RE = re.compile(r"-[eE]nc(?:oded)?[Cc]ommand\b|/[eE]:\b|-enc\b", re.IGNORECASE)
...
def _normalize_command(command: str) -> str:
    ...
    # Remove backtick + newline continuations
    text = command.replace("`\r\n", " ").replace("`\n", " ").replace("`\r", " ")
    # Also handle bare backtick at end of line
...
def _classify_powershell(command: str) -> tuple[str, str, List[str]]:
    ...
    # Invoke-expression → blocked (already caught by encoded/remote, but standalone also)
    ...
    # Git: safe for status/diff/log/show, confirm for others, blocked for destructive
    if exe == "git":
        if len(tokens) >= 2:
            sub = tokens[1]
    ...
    # Python: safe for reads, confirm for writes
    if exe in {"python", "py"}:
    ...
    # Default for unknown shell commands
    if _CHAIN_SEPARATORS.search(norm):
        return "confirm", "chained command", ["contains command chains or pipes"]

    return "safe", "unknown command — assumed safe", []
...
def assess_tool_call(tool_name: str, args: dict, cwd: str | None = None) -> SafetyDecision:
    ...
    The model proposes; this function decides.
    """
    if cwd is None:
        cwd = os.getcwd()
