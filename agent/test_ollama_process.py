...
class TestIsRunning:
    ...
    def test_returns_false_on_timeout(self):
        ...
        with patch(
            "agent.ollama_process.subprocess.run",
...
class TestWaitForReady:
    ...
    async def test_returns_false_on_timeout(self):
        ...
        with (
            patch("agent.ollama_process.ollama.list", side_effect=Exception("not ready")),
...
class TestEnsureRunning:
    async def test_already_running_skips_start(self):
        ...
        with (
            patch("agent.ollama_process.subprocess.run") as mock_run,
    ...
    async def test_not_running_calls_start(self):
        ...
        with (
            patch("agent.ollama_process.subprocess.run", side_effect=FileNotFoundError()),
            patch("agent.ollama_process.subprocess.Popen") as mock_popen,
